...
export interface ReportStats {
  durationMs: number;
  ruleCount: number;
  /** Number of problems rewritten by `--fix` before the remaining violations were collected. */
  fixed?: number;
}
...
const emphasize = (message: string): string =>
  message.replaceAll(/`[^`]*`|\d+(?:\.\d+)?(?:×\d+)?(?:px)?|\b(?:top|right|bottom|left)\b/g, (match) => pc.blue(match));
...
export function formatViolations(violations: Violation[], stats: ReportStats): string {
  ...
  if (violations.length === 0 && fixed === 0) return '';
  ...
  if (fixed > 0) lines.push(pc.green(`Fixed ${count(fixed, 'problem')}.`));
  lines.push(
