...
for (let i = 1; i <= 12; i++) {
  ...
  page.on('requestfailed', (r) => {
    ...
    console.log(`run ${i} REQFAIL ${r.url()} ${r.failure()?.errorText}`);
  });
  try {
    await page.goto(`http://127.0.0.1:${address.port}/print?print=true`, { waitUntil: 'networkidle', timeout: 15_000 });
  } catch {
  ...
  await page.locator('.print-slide-container').first().waitFor({ timeout: 15_000 });
  ...
  step('waitfor attrs');
  await Promise.all(page.frames().map((frame) => frame.waitForLoadState(undefined, { timeout: 15_000 })));
  step('frames');
